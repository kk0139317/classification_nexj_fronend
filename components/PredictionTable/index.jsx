...
const PredictionsTable = () => {
    ...
    useEffect(() => {
        ...
        axios.get('https://192.168.1.25:8000/api/predictions/')
            .then(response => {
                        ...
                        {predictions.map(prediction => (
                            <tr key={prediction.id} className="transition duration-300 ease-in-out transform hover:bg-blue-100 cursor-pointer">
                                <td className="-pr-4 pl-6 py-4 whitespace-nowrap">
                                    <img src={`https://192.168.1.25:8000/${prediction.image}`} alt="Prediction" className="h-16 w-16 object-cover rounded-full border-2 border-blue-500" onClick={() => openPopup(prediction)} />
                                </td>
                    ...
                    <div className="relative bg-white rounded-lg p-8 max-w-3xl w-full shadow-lg">
                        ...
                        <div className="flex flex-col sm:flex-row">
                            <div className="flex-shrink-0">
                                <img src={`https://192.168.1.25:8000/${selectedPrediction.image}`} alt="Prediction" className="h-64 w-full sm:w-64 object-cover rounded-lg border-2 border-gray-300" />
                            </div>
                            <div className="sm:ml-6 mt-4 sm:mt-0 space-y-2">
                                ...
                                <p className="font-semibold text-lg">URL: <a href={`https://192.168.1.25:8000/${selectedPrediction.image}`} className="text-blue-600 hover:underline break-all" download >{`http://127.0.0.1:8000/${selectedPrediction.image}`}</a></p>
                            </div>
                        </div>
                    </div>
                </div>
